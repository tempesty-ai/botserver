    ...
    try:
        while True:
            ...
            if status == 'completed':
                ...
                responses = [message.content[0].text.value for message in message_list.data]

                # 불필요한 소스 정보를 제거
                clean_responses = [remove_source_info(response) for response in responses]
...
def remove_source_info(text):
    # 특수 문자나 불필요한 정보 제거
    # 패턴을 사용하여 【4:1 형식과 † 기호 이후를 제거
    clean_text = re.sub(r'【.*?】', '', text)  # 【4:1 형식 제거
    clean_text = clean_text.split('†')[0].strip()  # † 이후 제거
    return clean_text
...
def slash_command():
    ...
    return jsonify({
        "response_type": "ephemeral",
        "text": f"'{user_input}'에 대한 답변을 생성 중입니다. \n잠시만 기다려주세요. \n챗봇에 답변 내용은 틀릴 수도 있으니 참고만 해주세요."
    })
